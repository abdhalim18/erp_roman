...
BEGIN
  -- Batas tanggal kedaluwarsa (14 hari dari sekarang)
  v_threshold_date := CURRENT_DATE + INTERVAL '14 days';
  ...
  LOOP
    ...
    FOR v_batch IN
      ...
      WHERE product_id = (v_item->>'product_id')::UUID
        AND quantity > 0
        AND (expiry_date IS NULL OR expiry_date > v_threshold_date)
    ...
    -- Jika setelah loop masih ada sisa (berarti stok valid tidak cukup)
    IF v_remaining > 0 THEN
      RAISE EXCEPTION 'Stok valid (tidak kedaluwarsa dalam 14 hari) tidak mencukupi untuk %', v_item->>'product_name';
    END IF;
